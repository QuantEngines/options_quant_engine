...
subtitle: "Architecture Reconstructed From Code"
author: "Pramit Dutta"
date: "March 2026"
---

<div class="memo-cover">
<div class="cover-kicker">Architecture Reference</div>
<h1 class="cover-title">Options Quant Engine</h1>
<div class="cover-subtitle">Architecture Reconstructed From Code</div>
<div class="cover-rule"></div>
<p class="cover-summary">The architecture the repository actually implements today, reconstructed from imports and orchestration code — not an idealized target architecture.</p>
<div class="cover-meta">
<div><span>Author</span>Pramit Dutta</div>
<div><span>Organization</span>Quant Engines</div>
<div><span>Date</span>March 2026</div>
<div><span>Document</span>Architecture Reference</div>
<div><span>Scope</span>True entrypoints, layer map, signal path, and implementation drift</div>
</div>
</div>
...
- dispatch through the pluggable predictor factory based on `PREDICTION_METHOD` config
- support runtime predictor swaps for backtesting and research

Available prediction methods:

- `blended` (default): rule + ML weighted blend (current production pipeline)
- `pure_ml`: ML leg only, rule leg suppressed
- `pure_rule`: rule leg only, ML leg suppressed
- `research_dual_model`: GBT ranking + LogReg calibration from research models
- `research_decision_policy`: dual-model + decision-policy overlay (ALLOW/BLOCK/DOWNGRADE)
- custom predictors via `register_predictor(name, cls)`

Separation quality:

- Good.
- Probability is a real intermediate layer, not hidden inside trade scoring.
- The `MovePredictor` Protocol and factory pattern provide a clean extension point.
...
3. Build probability state with `engine.trading_support.probability._compute_probability_state(...)` (dispatches through the pluggable predictor factory when `PREDICTION_METHOD` is non-default).
...
| Move probability | `engine.trading_support.probability._compute_probability_state(...)` dispatched via `engine.predictors.factory` |
...
- `market_state`
- `probability_state` (now backed by pluggable predictor architecture)
- `signal_state`
- overlay layers
- runtime sinks
- research dataset
- parameter runtime context
- predictor protocol and factory (`engine/predictors/`)

Those seams are real and visible in code. The architecture does not need to be invented from scratch; it needs to be hardened around those existing boundaries. The pluggable predictor architecture demonstrates that the probability seam is already production-grade and extensible.
